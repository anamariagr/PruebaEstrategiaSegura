document.addEventListener("DOMContentLoaded", function () {
  ...
  function applyPriority(val) {
    ...
    if (priorityBadge) {
      ...
      priorityBadge.classList.add(
        "priority-" + val.toLowerCase().replace("í", "i"),
      );
    }
  ...
  function updateOptionCards() {
    Object.values(optionMap).forEach(
      (el) => el && el.classList.remove("selected"),
    );
  ...
  function selectCurrentPriority() {
    const currentPriority =
      localStorage.getItem("casePriority") || estadoText.textContent.trim();
    const currentRadio = priorityRadios.find(
      (r) => r.value === currentPriority,
    );
  ...
  priorityRadios.forEach((radio) =>
    radio.addEventListener("change", updateOptionCards),
  );
